...
if (isset($_POST['eparentIdlevelone']) && isset($_POST['edittitle']) && isset($_POST['etype'])  && isset($_POST['reportId'])) {
    ...
    $sql   = "UPDATE reports_level1 SET parentId = $eparentIdlevelone,title = '$edittitle',type = $etype WHERE reportId = $reportId";
    ...
    if ($rowsAffected == 1) {
        ...
        $academicQuery = mysqli_query($conn, "SELECT rl.reportId,rl.title as mtitle,rt.title as title,rt.reportId as sreportId FROM reports_level1 rl INNER JOIN reports_title_master rt ON rt.reportId = rl.parentId WHERE rl.reportId = $reportId");
